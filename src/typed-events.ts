...
export function TypedEventEmitterClass<
  TEventRecord,
  TEmitRecord = TEventRecord,
  TEmitter = any,
>(
  emitter: TEmitter,
): Type<TypedEventEmitter<TEmitter, TEventRecord, TEmitRecord>> {
  return emitter as unknown as Type<
    TypedEventEmitter<TEmitter, TEventRecord, TEmitRecord>
  >;
}
...
type _TypedEventEmitter<
  ...
  TEventRecord,
  TEmitRecord,
  TEventEmitterOverride = _EventEmitterOverride<TEventRecord, TEmitRecord>,
  // Pick intersection keys of TEmitter and EventEmitterOverride
  TKeysToOverride extends keyof TEventEmitterOverride = Extract<
    keyof TEventEmitterOverride,
    keyof TEmitter
  >,
> =
...
type _EventEmitterOverride<
  TEventRecord,
  TEmitRecord = TEventRecord,
  TEventRecordKeys extends keyof TEventRecord = Extract<
    keyof TEventRecord,
    FunctionKeys<TEventRecord>
  >,
  TEmitRecordKeys extends keyof TEmitRecord = Extract<
    keyof TEmitRecord,
    FunctionKeys<TEmitRecord>
  >,
> = {
  ...
  addListener(
    event: typeof _assignmentCompatibilityHack,
    listener: Listener,
  ): void;
  ...
  emit<P extends TEmitRecordKeys, TThis>(
    this: TThis,
    event: P,
    ...args: _ListenerParameters<TEmitRecord[P]>
  ): boolean;
  ...
  removeListener<TThis>(
    this: TThis,
    event: TEventRecordKeys,
    listener: Listener,
  ): TThis;
  removeListener(
    event: typeof _assignmentCompatibilityHack,
